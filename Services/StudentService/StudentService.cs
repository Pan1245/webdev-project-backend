    ...
    public class StudentService : IStudentService
    {
        ...
        public async Task<ServiceResponse<List<GetStudentDto>>> DeleteStudent(int id)
        {
            ...
            try
            {
                ...
                if (dbParent is null)
                    throw new Exception($"Parent with Student ID '{id}' not found");

                _context.Parents.Remove(dbParent);

                var dbAddress = await _context.Addresses.FirstAsync(a => a.studentId == id);
                if (dbParent is null)
                    throw new Exception($"Address with Student ID'{id}' not found");
        ...
        public async Task<ServiceResponse<List<GetStudentCourseWithClassesDto>>> GetStudentCourseWithClassesByMe()
        {
        ...
        public async Task<ServiceResponse<GetStudentDto>> UpdateStudent(UpdateStudentDto updatedStudent)
        {
            ...
            try
            {
                ...
                var parent = await _context.Parents.FirstOrDefaultAsync(p => p.studentId == updatedStudent.id);
                if (parent is null)
                    throw new Exception($"Parent with student ID '{updatedStudent.id}' not found.");

                _mapper.Map(updatedStudent.parent, parent);
                ...
                var address = await _context.Addresses.FirstOrDefaultAsync(a => a.studentId == updatedStudent.id);
                if (address is null)
                    throw new Exception($"Address with student ID '{updatedStudent.id}' not found.");

                _mapper.Map(updatedStudent.address, address);
